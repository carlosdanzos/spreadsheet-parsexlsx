...
sub _get_text_and_rich_font_by_cell {
    ...
    my @nodes_r = $si->find_nodes('.//r');
    if( scalar @nodes_r  > 0) {
        #check r nodes if we have
        foreach my $node_r ( @nodes_r ) {                 
            foreach ($node_r->children) {
                
                my $string_lengt = length($string_text);
                if('t' eq $_->name) {
                    $string_text .= $_->text;
                } elsif ( 'rPr' eq $_->name ) { #here is where we check rich format
                    ...
                    my %format_text = (
                                       ...
                                       Strikeout        => 0,
                                       Super            => 0);
                    foreach my $node_format ($_->children) {
                        #check all the children with formats
                        if ( 'sz' eq $node_format->name ) {
                            $format_text{Height} = $node_format->att('val');
                        }
                        elsif ( 'color' eq $node_format->name ) {
                            $format_text{Color} = $self->_color($theme_colors, $node_format);
                        } elsif ( 'rFont' eq $node_format->name ) {
                            $format_text{Name} = $node_format->att('val');
                        } elsif ( 'b' eq $node_format->name ) {
                            $format_text{Bold} = 1;
                        } elsif ( 'i' eq $node_format->name ) {
                            $format_text{Italic} = 1;
                        } elsif ( 'u' eq $node_format->name ) {
                        ...
                        } elsif ( 'strike' eq $node_format->name ) {
                            $format_text{Strikeout} = 1;
                        } elsif ( 'vertAlign' eq $node_format->name ) {
                            if('superscript' eq $node_format->att('val')) {
                                $format_text{Super} = 1;
                            } elsif ('subscript' eq $node_format->att('val')) {
                                $format_text{Super} = 2;
                            }
                        }
                    }
                    push @{$rich_font_by_cell}, [$string_lengt, Spreadsheet::ParseExcel::Font->new(%format_text)];
                }
            }
        }    
    } else {
        $string_text = join( '', map { $_->text } $si->find_nodes('.//t') );
    }
    return (String => $string_text, Rich => $rich_font_by_cell);
}
...
sub _parse_shared_strings {
    ...
    return (Rich => \%richfonts, PkgStr => $PkgStr);
}
